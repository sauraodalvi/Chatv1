...
import { initializeStoryArc, generateWritingInstructions, getScenarioData } from './utils/storyArcUtils';
...
function App() {
  ...
  const startChat = (roomName, characters, openingPrompt = '', background = '', theme = '') => {
    ...
    if (scenarioData && scenarioData.initialMessages && scenarioData.initialMessages.length > 0) {
      console.log("Found scenario data with initial messages:", scenarioData.initialMessages.length);
      ...
      scenarioData.initialMessages.forEach((msgData, index) => {
        ...
        const characterIndex = characters.findIndex(char => char.name === msgData.speaker);

        if (characterIndex !== -1) {
          console.log(`Found character ${msgData.speaker} at index ${characterIndex}`);
          ...
          const instructions = generateWritingInstructions(initialStoryArc, character);
          ...
          initialMessages.push({
            ...
            timestamp: new Date(new Date().getTime() + (1000 * (index + 1))).toISOString(),
            writingInstructions: instructions
          });
        } else {
          console.warn(`Character ${msgData.speaker} not found in characters array!`);
      ...
      const greeting = firstCharacter.opening_line || `Hello! I'm ${firstCharacter.name}. How can I help you today?`;

      // Generate appropriate writing instructions
      const instructions = generateWritingInstructions(initialStoryArc, firstCharacter);
