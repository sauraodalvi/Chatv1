...
import { Sparkles, ArrowRight, RefreshCw, ArrowLeft, Search, Wand2, ChevronRight, X } from 'lucide-react';
...
const QuickScenarioSelector = ({ onStartChat, onBack }) => {
  ...
  const scenarioCategories = [
    { name: 'Fantasy', keywords: 'fantasy adventure magic quest', icon: '🧙‍♂️', description: 'Magical journeys' },
    { name: 'Sci-Fi', keywords: 'space scifi future technology', icon: '🚀', description: 'Cosmic wonders' },
    { name: 'Mystery', keywords: 'mystery detective crime investigation', icon: '🔍', description: 'Perplexing cases' },
    { name: 'Historical', keywords: 'historical medieval renaissance ancient', icon: '📜', description: 'Bygone eras' },
    { name: 'Romance', keywords: 'romance love relationship modern', icon: '❤️', description: 'Love stories' },
    { name: 'Horror', keywords: 'horror scary supernatural survival', icon: '👻', description: 'Terrifying scenarios' },
    { name: 'Combat', keywords: 'combat battle arena warrior sword duel', icon: '⚔️', description: 'Fierce battles' },
    { name: 'Superhero', keywords: 'superhero powers hero villain', icon: '🦸‍♀️', description: 'Extraordinary heroes' },
  ];
  ...
  const generateScenarios = (keywordString) => {
    ...
    const franchiseThemes = {
      "star wars": {
        ...
        settings: ["Tatooine desert", "Death Star", "Rebel base", "Imperial fleet", "Jedi temple", "Coruscant", "Endor moon", "Hoth ice planet"],
        characters: ["Jedi Knight", "Sith Lord", "smuggler", "rebel pilot", "Imperial officer", "bounty hunter", "droid", "Wookiee"],
        items: ["lightsaber", "blaster", "starship", "the Force", "hyperdrive", "droid", "Millennium Falcon"],
        conflicts: ["Rebellion against Empire", "Jedi vs Sith", "smuggling operation", "space battle", "lightsaber duel", "Imperial invasion"],
        themes: ["the Force", "good vs evil", "rebellion", "destiny", "redemption", "legacy"]
      },
      "lord of the rings": {
        universe: "Middle-earth",
        settings: ["Shire", "Mordor", "Rivendell", "Gondor", "Rohan", "Moria mines", "Lothlorien forest"],
        characters: ["hobbit", "wizard", "elf", "dwarf", "ranger", "orc", "Ringwraith"],
        items: ["the One Ring", "Elvish blade", "mithril armor", "wizard staff", "palantir"],
        conflicts: ["quest to destroy the Ring", "battle for Middle-earth", "siege of Gondor", "corruption by the Ring"],
        themes: ["power and corruption", "fellowship", "heroism", "sacrifice", "journey"]
      },
      "harry potter": {
        universe: "Wizarding World",
        settings: ["Hogwarts School", "Diagon Alley", "Ministry of Magic", "Forbidden Forest", "Hogsmeade village"],
        characters: ["wizard student", "professor", "magical creature", "Auror", "Death Eater", "house-elf"],
        items: ["wand", "broomstick", "potion", "magical artifact", "Marauder's Map", "invisibility cloak"],
        conflicts: ["wizard duel", "magical tournament", "fight against dark magic", "Quidditch match"],
        themes: ["magic", "friendship", "coming of age", "prejudice", "sacrifice"]
      },
      "game of thrones": {
        universe: "Westeros",
        settings: ["King's Landing", "Winterfell", "The Wall", "Iron Islands", "Dorne", "Braavos"],
        characters: ["noble lord", "knight", "sellsword", "wildling", "maester", "assassin"],
        items: ["Valyrian steel", "dragon egg", "wildfire", "poison", "direwolf"],
        conflicts: ["battle for the throne", "political intrigue", "house rivalry", "White Walker invasion"],
        themes: ["power", "betrayal", "honor", "survival", "family legacy"]
      },
      "marvel": {
        universe: "Marvel Universe",
        settings: ["New York City", "Avengers Tower", "secret lab", "alien planet", "Wakanda", "alternate dimension"],
        characters: ["superhero", "supervillain", "S.H.I.E.L.D. agent", "mutant", "scientist", "cosmic entity"],
        items: ["super serum", "Infinity Stone", "advanced tech", "magical artifact", "vibranium shield"],
        conflicts: ["alien invasion", "superhero civil war", "stopping a villain's plan", "saving the universe"],
        themes: ["responsibility", "heroism", "teamwork", "identity", "sacrifice"]
      }
    };
    ...
    const detectFranchise = (keywords) => {
      ...
      if (lowerKeywords.includes("lord") && lowerKeywords.includes("rings")) {
        return { franchise: "lord of the rings", themes: franchiseThemes["lord of the rings"] };
      }
      if (lowerKeywords.includes("harry") && lowerKeywords.includes("potter")) {
        return { franchise: "harry potter", themes: franchiseThemes["harry potter"] };
      }
      if (lowerKeywords.includes("game") && lowerKeywords.includes("thrones")) {
        return { franchise: "game of thrones", themes: franchiseThemes["game of thrones"] };
      }
      if (lowerKeywords.includes("marvel") || lowerKeywords.includes("avengers")) {
    ...
    const createFranchiseScenario = (franchise, themes, focus) => {
      ...
      const baseElements = {
        ...
        setting: themes.settings[Math.floor(Math.random() * themes.settings.length)],
        character: themes.characters[Math.floor(Math.random() * themes.characters.length)],
      ...
      if (focus === 'action') {
        const conflict = themes.conflicts[Math.floor(Math.random() * themes.conflicts.length)];
        focusElements = {
    ...
    const createVariation = (baseKeywords, focus) => {
      const keywords = baseKeywords.toLowerCase().split(/[,\s]+/).filter(k => k.length > 2);

      // Add scenario-specific variations
      const variations = {
        action: ['battle', 'combat', 'fight', 'action', 'conflict', 'challenge'],
        mystery: ['mystery', 'investigation', 'secret', 'puzzle', 'intrigue', 'conspiracy'],
        character: ['character', 'personal', 'emotional', 'relationship', 'development', 'backstory'],
        setting: ['world', 'environment', 'location', 'atmosphere', 'landscape', 'setting']
      };
    ...
    setTimeout(() => {
      ...
      if (franchiseMatch) {
        ...
        scenarios.forEach(scenario => {
          scenario.characters = generateCharactersForScenario(franchiseKeywords, 3);
        });
      }
      else {
        ...
        scenarios[0].characters = generateCharactersForScenario(actionKeywords, 3);
        scenarios[1].characters = generateCharactersForScenario(mysteryKeywords, 3);
        scenarios[2].characters = generateCharactersForScenario(characterKeywords, 3);
      }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex border-b mb-4">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium flex items-center ${activeTab === 'categories' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'}`}
            >
          ...
          {activeTab === 'categories' && (
            <>
              ...
              <div className="mb-6">
                <h3 className="text-sm font-medium mb-3">Inspired by Popular Media:</h3>
          ...
          {activeTab === 'custom' && (
            <div className="space-y-4">
              ...
              <div className="text-xs text-muted-foreground mb-4">
                <p>Try to be specific about the setting, time period, and mood. For example:</p>
                <ul className="list-disc pl-5 mt-1 space-y-1">
                  <li>"Post-apocalyptic desert with mutants and scarce resources"</li>
                  <li>"Victorian London mystery with supernatural elements"</li>
                  <li>"Space station diplomatic mission with alien delegates"</li>
                </ul>
              </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Based on: "{keywords}"</p>
            </div>
            <button
            ...
            {generatedScenarios.map((scenario, index) => (
              ...
              >
                ...
                <div
                  ...
                  style={scenario.background.startsWith('linear-gradient') ?
                ...
                >
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                    {scenario.franchise ? (
                      <div className="bg-yellow-500/20 text-yellow-500 backdrop-blur-sm px-2 py-0.5 rounded-full text-xs font-medium">
                        {scenario.franchise.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                      </div>
                    ) : (
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 sm:hidden">
                    <h3 className="text-base font-bold text-foreground drop-shadow-sm line-clamp-1">{scenario.title}</h3>
                  </div>
                </div>
                ...
                <div className="p-3 flex-1 flex flex-col sm:flex-row sm:items-center sm:gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="hidden sm:block text-base font-bold mb-1">{scenario.title}</h3>
                    ...
                    <div className="mb-2">
                      ...
                      <div className="mt-1 text-xs">
                        {scenario.actionFocus && (
                          <span className="text-red-500 font-medium">Focus on combat and challenges</span>
                        )}
                        {scenario.mysteryFocus && (
                          <span className="text-purple-500 font-medium">Uncover secrets and solve puzzles</span>
                        )}
                        {scenario.characterFocus && (
                          <span className="text-blue-500 font-medium">Deep character interactions</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex -space-x-1">
                        {scenario.characters.slice(0, 3).map((char, i) => (
                          <div key={i} className="w-5 h-5 rounded-full bg-secondary/50 border border-background flex items-center justify-center text-xs">
                            {char.type === 'fantasy' && '✨'}
                            {char.type === 'scifi' && '🔭'}
