...
const getCharacterVoiceTemplate = (character) => {
  ...
  if (mood) {
    ...
    if (moodLower.includes('angry') || moodLower.includes('furious')) {
      defaultTemplate.rules.push("Express frustration or anger in your responses");
      ...
      defaultTemplate.examples.push("This is fantastic! I couldn't be more thrilled!");
      ...
      defaultTemplate.examples.push("I suppose it doesn't really matter in the end...");
    }
  }
 ...
 */
const filterResponseForScenario = (response, character, storyArc, voiceTemplate) => {
  if (!response || !character) return response;

  let filteredResponse = response;

  // Check for forbidden phrases from the character's voice template
  if (voiceTemplate && voiceTemplate.forbidden_phrases && voiceTemplate.forbidden_phrases.length > 0) {
  ...
  if (storyArc) {
    ...
    if ((storyArc.currentPhase === 'conflict' || storyArc.currentPhase === 'climax') &&
        storyArc.currentTension === 'high' || storyArc.currentTension === 'very high') {

      // If response is too casual for an urgent situation, add urgency
      if (!containsUrgencyTerms(filteredResponse) && !filteredResponse.includes('*')) {
        filteredResponse = addUrgencyToResponse(filteredResponse, character, storyArc);
      }
    }

    // Ensure response references the environment if it doesn't already
    if (!filteredResponse.includes('*') && Math.random() < 0.3) {
      filteredResponse = addEnvironmentalContext(filteredResponse, character, storyArc);
    }
  }
...
const containsUrgencyTerms = (response) => {
  const urgencyTerms = [
    'quick', 'fast', 'hurry', 'now', 'immediately', 'urgent', 'emergency',
    'danger', 'threat', 'critical', 'priority', 'crucial', 'vital',
    'attack', 'defend', 'protect', 'evacuate', 'run', 'hide', 'take cover'
  ];
...
const addEnvironmentalContext = (response, character, storyArc) => {
  ...
  const actions = environmentalActions[type] && environmentalActions[type][phase]
    ? environmentalActions[type][phase]
    : ["*looks around*", "*pauses briefly*", "*considers the situation*", "*thinks for a moment*"];
...
const getCharacterAppropriateReplacement = (phrase, character, storyArc) => {
  ...
  for (const key of Object.keys(replacements)) {
    const similarity = stringSimilarity(phrase.toLowerCase(), key.toLowerCase());
  ...
  if (closestMatch && highestSimilarity > 0.5) {
    const type = character.type || 'default';
    return replacements[closestMatch][type] || replacements[closestMatch].default;
  }
...
export const generateCharacterResponse = (character, message, chatHistory, writingInstructions = null, chatRoom = null, relationships = []) => {
  ...
  const { name, mood, type, personality = {}, voiceStyle = '', catchphrases = [], role = '' } = character;
  ...
  const messageStyle = {
    voiceStyle: voiceStyle || `${mood && mood.toLowerCase() || 'neutral'} and ${type || 'conversational'}`,
    mood: mood || 'Neutral',
  ...
  const storyArc = hasStoryArcContext ? JSON.parse(instructions.storyArc) : null;

  // Analyze recent dialogue history
  const dialogueAnalysis = analyzeDialogueHistory(chatHistory, 6);

  // Generate narrative guidance
  const narrativeGuidance = generateNarrativeGuidance(dialogueAnalysis, storyArc, character);

  // Generate response format
  const responseFormat = generateResponseFormat(character, narrativeGuidance, storyArc);

  // Generate scene description
  const sceneDescription = generateSceneDescription(character, storyArc, chatHistory, 'response');
  ...
  let catchphrase = "";
  if (messageStyle.catchphrases && messageStyle.catchphrases.length > 0 && Math.random() < 0.2) {
    catchphrase = messageStyle.catchphrases[Math.floor(Math.random() * messageStyle.catchphrases.length)];
  ...
  if (messageStyle.voiceStyle && Math.random() < 0.3) {
    ...
    if (voiceStyleLower.includes('formal') || voiceStyleLower.includes('proper')) {
      voiceStyleInfluence = " I must say, this matter deserves proper consideration.";
    } else if (voiceStyleLower.includes('blunt') || voiceStyleLower.includes('direct')) {
      voiceStyleInfluence = " Let's not waste time with pleasantries.";
    } else if (voiceStyleLower.includes('poetic') || voiceStyleLower.includes('flowery')) {
      voiceStyleInfluence = " The tapestry of words we weave tells its own story.";
    } else if (voiceStyleLower.includes('technical') || voiceStyleLower.includes('scientific')) {
      voiceStyleInfluence = " The empirical evidence supports this conclusion.";
    } else if (voiceStyleLower.includes('mysterious') || voiceStyleLower.includes('cryptic')) {
      voiceStyleInfluence = " There are layers of meaning here that few will understand.";
    }
  }
  ...
  if (messageStyle.role && Math.random() < 0.25) {
    ...
    if (roleLower.includes('leader') || roleLower.includes('commander')) {
      roleBasedContent = " As someone responsible for others, I take this matter seriously.";
    } else if (roleLower.includes('mentor') || roleLower.includes('teacher')) {
      roleBasedContent = " Let me share what I've learned about this over the years.";
    } else if (roleLower.includes('scientist') || roleLower.includes('researcher')) {
      roleBasedContent = " My research in this area suggests several interesting possibilities.";
    } else if (roleLower.includes('warrior') || roleLower.includes('fighter')) {
      roleBasedContent = " In battle, this kind of thinking could mean the difference between life and death.";
    } else if (roleLower.includes('healer') || roleLower.includes('doctor')) {
      roleBasedContent = " I've seen how this affects people's wellbeing firsthand.";
    }
  }
  ...
  if (storyArc) {
    finalResponse = filterResponseForScenario(finalResponse, character, storyArc, messageStyle.voiceTemplate);
  }
  ...
  if (sceneDescription && narrativeGuidance) {
    finalResponse = formatCharacterResponse(finalResponse, responseFormat, sceneDescription);
  }
...
export const generateCharacterInteraction = (respondingCharacter, targetCharacterName, targetMessage, chatHistory, relationships = []) => {
  ...
  const simpleStoryArc = {
    theme: type || 'general',
    currentPhase: hasDanger ? 'conflict' : (hasQuestion ? 'discovery' : 'introduction'),
    currentTension: hasDanger ? 'high' : 'medium',
    currentGoal: hasDanger ? 'address immediate threat' : 'maintain conversation flow'
  };
  ...
  // Generate narrative guidance
  const narrativeGuidance = generateNarrativeGuidance(dialogueAnalysis, simpleStoryArc, respondingCharacter);

  // Generate response format
  const responseFormat = generateResponseFormat(respondingCharacter, narrativeGuidance, simpleStoryArc);

  // Generate scene description
  const sceneDescription = generateSceneDescription(respondingCharacter, simpleStoryArc, chatHistory, 'interaction');
  ...
  if (type) {
    ...
    // Filter the response
    finalResponse = filterResponseForScenario(finalResponse, respondingCharacter, simpleStoryArc, voiceTemplate);
  }
