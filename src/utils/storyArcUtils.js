...
const scenarioDataRepository = {
  ...
  "The Lost Artifact": {
    ...
    initialContext: 'A group of adventurers has gathered at the edge of the Forbidden Forest. Ancient texts speak of a powerful artifact hidden within the ruins of a forgotten civilization. Dark forces are also seeking the artifact, and time is running out.',
    initialNarration: "The air is thick with magic as your party stands at the entrance to the ancient forest. Mist swirls around gnarled trees, and distant whispers seem to call from within the depths. The map you've been following has led you here, to the first step of your quest for the legendary artifact.",
    initialMessages: [
      {
        speaker: "Elara Moonwhisper",
        message: "*traces glowing runes in the air, creating a protective ward around the group*",
        isAction: true
      },
      {
        speaker: "Elara Moonwhisper",
        message: "The forest's magic is ancient and unpredictable. Stay close, and whatever you do, don't stray from the path. The artifact's energy calls to me... it's definitely in there, but we're not the only ones seeking it."
      }
    ],
    phaseTransitions: {
      'introduction': {
  ...
  "Space Station Omega": {
    theme: "scifi",
    initialPhase: 'introduction',
    initialGoal: 'Investigate the mysterious signal coming from the abandoned station',
    initialTension: 'building',
    keyLocations: ['Command Center', 'Research Lab', 'Maintenance Tunnels'],
    initialContext: 'Your crew has been dispatched to investigate a mysterious signal emanating from Space Station Omega, which was abandoned years ago after a catastrophic incident. The nature of the signal is unknown, but it matches no known pattern in the database.',
    initialNarration: "The airlock hisses as your ship completes its docking procedure with Space Station Omega. Through the viewport, you can see the station's exterior is damaged and dark, with only a few emergency lights still functioning. The mysterious signal continues to pulse from somewhere deep within the station.",
    initialMessages: [
      {
        speaker: "Commander Zax",
        message: "*checks the readings on his wrist device, frowning at the data*",
        isAction: true
      },
      {
        speaker: "Commander Zax",
        message: "Atmospheric readings show the station's life support is functioning at minimal capacity. We have approximately six hours before our oxygen reserves become critical. Stay alert - the last crew disappeared without a trace, and that signal doesn't match anything in our database. Standard protocol: weapons ready, comms open at all times."
      }
    ],
    phaseTransitions: {
      'introduction': {
...
export const getScenarioData = (scenarioTitle, scenarioTheme) => {
  ...
  if (scenarioDataRepository[scenarioTitle]) {
    return scenarioDataRepository[scenarioTitle];
  }

  // If not found by title, look for scenarios with matching theme
  const scenariosByTheme = Object.values(scenarioDataRepository)
    .filter(scenario => scenario.theme === scenarioTheme);
...
export const initializeStoryArc = (scenarioTitle, scenarioPrompt, scenarioTheme) => {
  ...
  if (scenarioData) {
    return {
      ...
      currentContext: scenarioData.initialContext || scenarioPrompt,
    };
  }

  // If no specific scenario data is found, use theme-based defaults
  if (scenarioTheme === "fantasy") {
    return {
      ...storyArc,
      currentPhase: 'introduction',
  ...
  } else if (scenarioTheme === "superhero") {
    return {
      ...storyArc,
      currentPhase: 'conflict',
      currentTension: 'high',
      currentContext: 'Heroes have assembled to face a growing threat. The situation is tense as they prepare to defend innocent lives.',
    };
  } else if (scenarioTheme === "horror") {
    return {
      ...storyArc,
      currentPhase: 'discovery',
      currentTension: 'building',
      currentContext: 'Something feels wrong. The first signs of danger have appeared, but the true horror remains hidden.',
    };
  }
...
export const updateStoryArc = (currentArc, recentMessages, messageCount = 5) => {
  ...
  if (scenarioData && scenarioData.phaseTransitions &&
  ...
  if (currentArc.theme === 'superhero') {
    ...
    } else if (combinedContent.includes('final') || combinedContent.includes('confront')) {
      updatedArc.currentGoal = 'Confront the main threat and save the day';
    }
  } else if (currentArc.theme === 'fantasy') {
    if (combinedContent.includes('quest') || combinedContent.includes('journey')) {
      updatedArc.currentGoal = 'Begin the quest and gather resources';
    } else if (combinedContent.includes('clue') || combinedContent.includes('map')) {
      updatedArc.currentGoal = 'Follow the trail to the next location';
    } else if (combinedContent.includes('battle') || combinedContent.includes('fight')) {
      updatedArc.currentGoal = 'Overcome the immediate threat';
    } else if (combinedContent.includes('artifact') || combinedContent.includes('treasure')) {
      updatedArc.currentGoal = 'Secure the artifact before it falls into the wrong hands';
    }
  } else if (currentArc.theme === 'scifi') {
    if (combinedContent.includes('scan') || combinedContent.includes('analyze')) {
      updatedArc.currentGoal = 'Analyze the anomaly and gather data';
    } else if (combinedContent.includes('malfunction') || combinedContent.includes('system')) {
      updatedArc.currentGoal = 'Repair critical systems before it\'s too late';
    } else if (combinedContent.includes('alien') || combinedContent.includes('contact')) {
      updatedArc.currentGoal = 'Establish contact with the unknown entity';
    } else if (combinedContent.includes('escape') || combinedContent.includes('evacuate')) {
      updatedArc.currentGoal = 'Evacuate before the situation becomes catastrophic';
    }
  }
...
export const generateWritingInstructions = (storyArc, character) => {
  ...
  const phaseInstructions = {
    introduction: {
      general: 'Focus on establishing character relationships and setting the scene. Keep tension moderate but building.',
      superhero: 'Establish your heroic persona and relationship to the team. Reference your powers or abilities when relevant.',
      fantasy: 'Establish your role in the party and your connection to the magical world. Reference your background and skills.',
      scifi: 'Establish your role on the crew and your technical expertise. Reference your specialized knowledge or equipment.'
    },
    discovery: {
      general: 'Focus on exploration and uncovering new information. Show curiosity and analytical thinking.',
      superhero: 'Analyze the situation and gather intelligence about the threat. Use your unique perspective to contribute insights.',
      fantasy: 'Interpret clues and magical signs. Use your knowledge of lore and legends to guide the party.',
      scifi: "Analyze sensor readings and anomalies. Use scientific reasoning to form hypotheses about what you're encountering."
    },
    conflict: {
      general: 'Emphasize action and immediate threats. Responses should be urgent and focused on the current battle.',
      superhero: 'Focus on protecting civilians and coordinating with teammates. Reference the immediate battle tactics.',
      fantasy: 'Focus on using your abilities in combat. Reference spells, weapons, or tactical advantages in the environment.',
      scifi: 'Focus on technical solutions to the crisis. Reference systems, equipment, or scientific principles that could help.'
    },
    planning: {
      general: 'Focus on strategy and analysis. Characters should be thinking about next steps and discussing the enemy\'s weaknesses.',
      superhero: 'Contribute strategic ideas based on your powers and experience. Reference previous battles or known weaknesses.',
      fantasy: 'Share knowledge of magical weaknesses or prophecies. Reference ancient texts or wisdom that might help.',
      scifi: 'Propose technical solutions based on data analysis. Reference scientific principles or technological capabilities.'
    },
    climax: {
      general: 'This is the high point of tension. Responses should be dramatic and impactful, with high stakes clearly communicated.',
      superhero: "Show determination in the face of overwhelming odds. Reference what's at stake and your resolve to protect others.",
      fantasy: 'Channel your full power against the final challenge. Reference prophecies, destinies, or ancient powers.',
      scifi: 'Execute the final plan with precision and adaptability. Reference critical systems, countdown timers, or final calculations.'
    },
    resolution: {
      general: 'Wind down the action and reflect on what happened. Focus on character growth and next steps.',
      superhero: 'Reflect on the victory and lessons learned. Reference the cost of the battle and plans for rebuilding.',
      fantasy: 'Celebrate the completion of the quest. Reference rewards, magical discoveries, or new bonds formed.',
      scifi: 'Analyze the mission outcomes and collected data. Reference scientific discoveries or technological advancements made.'
    }
  };
  ...
  if (storyArc.currentTension === 'high') {
    instructions.generalNotes += ' Maintain a sense of urgency in your responses.';
  } else if (storyArc.currentTension === 'very high') {
    instructions.generalNotes += ' Convey extreme urgency and high stakes in every response.';
  }
...
const generateUpdatedContext = (currentArc, recentContent) => {
  ...
  if (scenarioData) {
    ...
    if (scenarioData.phaseContexts && scenarioData.phaseContexts[currentArc.currentPhase]) {
